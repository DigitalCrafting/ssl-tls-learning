 ...
 * The problems:
 * - it keeps reallocating internal buffer essentially forever, which can crush the server,
 * - it reads one byte at a time from a socket,
 * - uses static variables
 */
char *read_line(int connection) {
    static int line_len = DEFAULT_LINE_LENGTH;
...
int main(int argc, char *argv[]) {
    ...
    local_addr.sin_addr.s_addr = htonl(
            INADDR_LOOPBACK); // This basically means, only accept connections from this computer, LOOPBACK is 127.0.0.1
